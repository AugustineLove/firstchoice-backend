...
exports.getRiderInsights = getRiderInsights;
exports.getRiderJobsPaginated = getRiderJobsPaginated;
...
async function getRiderInsights(riderId) {
    const rider = await prisma_1.prisma.rider.findUnique({
        where: { id: riderId },
    ...
    const [allOrders, allDeliveries, ordersLast30, deliveriesLast30, activeOrders, activeDeliveries, cancelledOrders, cancelledDeliveries,] = await Promise.all([
        prisma_1.prisma.order.findMany({
            where: { riderId },
        ...
        prisma_1.prisma.order.findMany({ where: { riderId, orderStatus: 'DELIVERED', updatedAt: { gte: since30 } }, select: { deliveryFee: true, updatedAt: true } }),
        prisma_1.prisma.deliveryRequest.findMany({ where: { assignedRiderId: riderId, status: 'DELIVERED', updatedAt: { gte: since30 } }, select: { estimatedFee: true, updatedAt: true } }),
        prisma_1.prisma.order.count({ where: { riderId, orderStatus: { in: ['RIDER_ASSIGNED', 'PICKED_UP'] } } }),
        prisma_1.prisma.deliveryRequest.count({ where: { assignedRiderId: riderId, status: { in: ['ACCEPTED', 'PICKED_UP', 'IN_TRANSIT'] } } }),
        prisma_1.prisma.order.count({ where: { riderId, orderStatus: 'CANCELLED' } }),
        prisma_1.prisma.deliveryRequest.count({ where: { assignedRiderId: riderId, status: 'CANCELLED' } }),
    ]);
    ...
    const todayEarnings = sum(deliveredOrders.filter(o => o.updatedAt >= startToday).map(o => o.deliveryFee)) +
        sum(deliveredDeliveries.filter(d => d.updatedAt >= startToday).map(d => d.estimatedFee));
    const weekEarnings = sum(deliveredOrders.filter(o => o.updatedAt >= startWeek).map(o => o.deliveryFee)) +
    ...
    for (const o of ordersLast30) {
        ...
        if (dayMap[key]) {
            dayMap[key].orders += o.deliveryFee ?? 0;
            dayMap[key].total += o.deliveryFee ?? 0;
            dayMap[key].jobCount += 1;
        }
    }
    for (const d of deliveriesLast30) {
        const key = startOfDay(d.updatedAt).toISOString().slice(0, 10);
        if (dayMap[key]) {
            dayMap[key].deliveries += d.estimatedFee ?? 0;
            dayMap[key].total += d.estimatedFee ?? 0;
            dayMap[key].jobCount += 1;
        }
    }
    ...
    for (const o of deliveredOrders)
        weekdayCounts[o.updatedAt.getDay()]++;
    for (const d of deliveredDeliveries)
        weekdayCounts[d.updatedAt.getDay()]++;
    ...
    const recentActivity = [
        ...allOrders.map(o => ({ id: o.id, kind: 'order', status: o.orderStatus, amount: o.deliveryFee, counterparty: o.customer?.name, note: o.vendor?.businessName, date: o.updatedAt })),
        ...allDeliveries.map(d => ({ id: d.id, kind: 'delivery', status: d.status, amount: d.estimatedFee, counterparty: d.customer?.name, note: d.type, date: d.updatedAt })),
...
async function getRiderJobsPaginated(riderId, filters) {
    const page = filters.page || 1;
    const limit = filters.limit || 20;
    const [orders, deliveries] = await Promise.all([
        filters.kind === 'delivery' ? [] : prisma_1.prisma.order.findMany({
            where: { riderId, ...(filters.status && { orderStatus: filters.status }) },
            include: { customer: { select: { name: true, phone: true } }, vendor: { select: { businessName: true } } },
            orderBy: { createdAt: 'desc' },
        }),
        filters.kind === 'order' ? [] : prisma_1.prisma.deliveryRequest.findMany({
            where: { assignedRiderId: riderId, ...(filters.status && { status: filters.status }) },
    ...
    const merged = [
        ...orders.map(o => ({
            id: o.id, kind: 'order', status: o.orderStatus,
        ...
        ...deliveries.map(d => ({
            id: d.id, kind: 'delivery', status: d.status,
